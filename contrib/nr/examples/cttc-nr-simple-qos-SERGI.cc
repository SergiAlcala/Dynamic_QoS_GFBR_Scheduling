...
{   /*
   ...
   // Create output directory if it does not exist
    ...
    // uint32_t lambda = 1000; // λ = packet arrival rate [packets per second]
    ...
    // std::cout << "UE count: " << ueContainer.GetN() << std::endl;
    // std::cout << "fiveQiVec size: " << fiveQiVec.size() << std::endl;

    NS_ABORT_MSG_IF(fiveQiVec.size() != ueContainer.GetN(),
                "5QI vector size mismatch");

    NS_ABORT_MSG_IF(ueTypeVec.size() != ueContainer.GetN(),
                "UE type vector size mismatch");

    
    NS_ABORT_MSG_IF(packetSizeVec.size() != ueContainer.GetN(),
                    "packetSizeVec size mismatch");

    NS_ABORT_MSG_IF(lambdaVec.size() != ueContainer.GetN(),
                    "lambdaVec size mismatch");
    ...
    //  IF WE WANT CUSTOM BWP, WE UNCOMMENT THIS PART AND CONFIGURE THE BWP IDS WE WANT TO USE FOR EACH TRAFFIC TYPE, AND THEN WE ASSIGN THEM TO THE ROUTING ALGORITHMS. OTHERWISE, ALL TRAFFIC GOES TO THE SAME DEFAULT BWP (ID 0).
...
// When all the configuration is done, explicitly call UpdateConfig ()
    ...
    {
        Ptr<Node> ueNode = ueContainer.Get(i);
        Ptr<NetDevice> ueDevice = ueNetDev.Get(i);
        Ipv4Address ueIp = ueIpIface.GetAddress(i);

        uint16_t dlPort = baseDlPort + i;

        // --------------------
        // Traffic sink (UE)
        // --------------------
        UdpServerHelper dlSink(dlPort);
        serverApps.Add(dlSink.Install(ueNode));

        // --------------------
        // Traffic source (Remote Host)
        // --------------------

        uint32_t pktSize = packetSizeVec[i];
        uint32_t lam = lambdaVec[i];
        UdpClientHelper dlClient;
        dlClient.SetAttribute("RemoteAddress", AddressValue(ueIp));
        dlClient.SetAttribute("RemotePort",UintegerValue(dlPort));
        dlClient.SetAttribute("MaxPackets", UintegerValue(0xFFFFFFFF));
        dlClient.SetAttribute("PacketSize", UintegerValue(pktSize));
        dlClient.SetAttribute("Interval", TimeValue(Seconds(1.0 / lam)));
        ...
        // QoS per UE
        // --------------------
       EpsBearer::Qci qci = Parse5qi(fiveQiVec[i]);

        bool isGbr = (ueTypeVec[i] == "GBR");

        if (isGbr)
        {
            GbrQosInformation qos;
            qos.gbrDl = gbrDlVec[i];
            qos.mbrDl = mbrDlVec[i];
            qos.gbrUl = gbrUl;
            qos.mbrUl = mbrUl;
    ...
    */
    ...
    for (auto i = stats.begin(); i != stats.end(); ++i)
    {
        ...
        if (i->second.rxPackets > 0)
        {
            ...
            // outFile << "  Configured GFBR DL: " << voiceBearer.gbrQosInfo.gbrDl/ 1e6 << " Mbps\n";
        }
        else
        {
