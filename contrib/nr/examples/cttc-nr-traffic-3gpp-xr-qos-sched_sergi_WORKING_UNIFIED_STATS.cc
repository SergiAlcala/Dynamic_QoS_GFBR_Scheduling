...
//     newTraffic = std::max(newTraffic, static_cast<uint64_t>(1)); // Set a minimum traffic to avoid issues with zero rate
...
//               Ptr<NrMacSchedulerOfdmaQos> sched,
//               Ptr<NrUeNetDevice> ueDev,
//               Ptr<PacketSink> sink,
...
//     if (!headerWritten)
//     {
//         unifiedFile << "Time(s),RNTI,TargetTraffic(Mbps),GFBR(Mbps),MeasuredThroughput(Mbps)\n";
//         headerWritten = true;
//     }

//     // 1. Calculate the throughput achieved in the LAST interval
//     double throughputMbps = 0.0;
//     if (sink)
//     {
//         uint64_t curRxBytes = sink->GetTotalRx();
//         throughputMbps = (curRxBytes - *lastRxBytes) * 8.0 / (interval * 1e6);
//         *lastRxBytes = curRxBytes; 
//     }

//     // 2. Fetch the traffic targets that were active during the LAST interval (to align the logs)
//     double pastTrafficMbps = (index > 0) ? (traffic[index - 1] / 1e6) : 0.0;
//     double pastGfbrMbps    = (index > 0) ? (gbr[index - 1] / 1e6) : 0.0;

//     // 3. Log everything synchronously (Now the columns match perfectly in time!)
...
//     // 4. Fetch and apply the target Traffic and GFBR for the NEXT interval
...
//     Simulator::Schedule(Seconds(interval),
//     [=]() {
//         UnifiedUpdate(video, sched, ueDev, sink, lastRxBytes, traffic, gbr, index + 1, interval, unifiedName);
//     });
// }
void
UnifiedUpdate(Ptr<TrafficGenerator3gppGenericVideo> video,
              Ptr<NrMacScheduler> sched,  // <--- Changed to generic base class
              Ptr<NrUeNetDevice> ueDev,
              Ptr<PacketSink> sink,
              uint64_t *lastRxBytes,
              std::vector<uint64_t> traffic,
              ...
              const std::string& unifiedName)
{
    ...
    // 3. Log everything synchronously
    unifiedFile << Simulator::Now().GetSeconds() << ","
                << rnti << ","
                << pastTrafficMbps << ","
                << pastGfbrMbps << ","
    ...
    // --- THE FLEXIBLE FIX ---
    // Safely attempt to cast to QoS or DPP. If it succeeds, apply GFBR. 
    // If you run PF, RR, or MR, these casts fail safely and do nothing!
...
if (!configFile.empty())
{
    ...
    // if ("Qos" == schedulerType)
    // {
        // Ptr<NrMacSchedulerOfdmaQos> qosSched =
        //     DynamicCast<NrMacSchedulerOfdmaQos>(baseSched);
    // }
    // else if ("DPP" == schedulerType)
    // {
    ...
    // else
    // {
    // NS_ABORT_MSG("Unsupported scheduler type");
    // }
...
for (uint32_t i = 0; i < dynamicUes.size(); ++i)
    {
        ...
        [=]() {
            UnifiedUpdate(ctx.video,
                   baseSched,
                   ctx.ueDev,
                   ueSink,         // <--- Passing the sink
                   lastRxTracker,  // <--- Passing the byte tracker
                   ctx.traffic,
                   ctx.gbr,
                   0u,
                   interval,
