            ...
            DoubleValue Datarate)
{
    ...
    for (uint32_t j = 0; j < currentUeClientApps.GetN(); j++)
    {
        ...
        if (video)
        {
            video->SetAttribute("DataRate", DoubleValue(Datarate));  // Mbps
            video->SetAttribute("Fps", UintegerValue(60));      // optional
        }
    }
...
main(int argc, char* argv[])
{
    ...
    bool isMx1 = true;
    bool useUdp = true;
    double distance = 450; // 450 original
    uint32_t rngRun = 1;
    double dppV = 0.0;
    bool enableVirtualQueue = true;

    double Datarate = 20; // Mbps, default value for the video traffic generator

    uint64_t CGgbrDL = 0; // default (kbps)
    ...
    cmd.AddValue("CGgbrDL", "The GBR to be configured for the CG traffic in kbps. Default is 0, which means that the CG traffic will be configured with the same GBR as the VR traffic.", CGgbrDL);
    cmd.AddValue("Datarate", "The datarate to be configured for the video traffic generator in Mbps. Default is 20 Mbps.", Datarate);
    cmd.Parse(argc, argv);
