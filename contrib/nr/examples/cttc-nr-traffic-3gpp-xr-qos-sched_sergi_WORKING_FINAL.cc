...
//     Ptr<TrafficGenerator3gppGenericVideo> video;
//     Ptr<NrUeNetDevice> ueDev;
//     Ptr<PacketSink> sink; // <--- Add this
//     std::vector<uint64_t> traffic;
//     std::vector<uint64_t> gbr;
//     uint32_t index;
//     uint16_t rnti; // Add this to store the RNTI explicitly
// };
...
// void
// UnifiedUpdate(Ptr<TrafficGenerator3gppGenericVideo> video,
//               Ptr<NrMacScheduler> baseSched,
//               Ptr<NrUeNetDevice> ueDev,
//               Ptr<PacketSink> sink,
//               Ptr<FlowMonitor> flowMon,
//               Ptr<Ipv4FlowClassifier> classifier,
//               UeIntervalStats *tracker,
//               std::vector<uint64_t> traffic,
//               std::vector<uint64_t> gbr,
//               uint32_t index,
//               double interval,
...
//     static std::ofstream unifiedFile(unifiedName, std::ios::out);
//     static bool headerWritten = false;    
    
//     if (!headerWritten)
//     {
//         // Added PacketRate(pps) and DropRate(%) to the CSV header
//         unifiedFile << "Time(s),RNTI,TargetTraffic(Mbps),GFBR(Mbps),MeasuredThroughput(Mbps),MeasuredDelay(ms),PacketRate(pps),DropRate,AllocatedRBGs\n";
//         headerWritten = true;
//     }
...
//     if (sink)
//     {
//         uint64_t curRxBytes = sink->GetTotalRx();
//         throughputMbps = (curRxBytes - tracker->lastRxBytes) * 8.0 / (interval * 1e6);
//         tracker->lastRxBytes = curRxBytes; 
//     }
...
//     if (flowMon && classifier)
//     {
//         Ptr<Ipv4> ipv4 = ueDev->GetNode()->GetObject<Ipv4>();
...
//             {
//                 uint64_t currentRxPkts = stat.rxPackets;
//                 uint64_t currentTxPkts = stat.txPackets;
//                 double currentDelaySum = stat.delaySum.GetSeconds();
                
//                 uint64_t rxInInterval = currentRxPkts - tracker->lastRxPackets;
//                 uint64_t txInInterval = currentTxPkts - tracker->lastTxPackets;
                
//                 // Packet Rate: Received packets per second
//                 packetRatePps = rxInInterval / interval;

//                 // Drop Rate: (Tx - Rx) / Tx for this specific interval
//                 if (txInInterval > 0) 
//                 {
//                     dropRateInterval = static_cast<double>(txInInterval - rxInInterval) / txInInterval;
//                 }
                
//                 // Mean Delay for packets received in this interval
//                 if (rxInInterval > 0) 
//                 {
//                     intervalDelayMs = ((currentDelaySum - tracker->lastDelaySumSec) / rxInInterval) * 1000.0;
//                 }
                
//                 tracker->lastRxPackets = currentRxPkts;
//                 tracker->lastTxPackets = currentTxPkts;
//                 tracker->lastDelaySumSec = currentDelaySum;
//                 break; 
//             }
//         }
//     }

//     double pastTrafficMbps = (index > 0) ? (traffic[index - 1] / 1e6) : 0.0;
//     double pastGfbrMbps    = (index > 0) ? (gbr[index - 1] / 1e6) : 0.0;
//     uint32_t rbgThisInterval = ns3::g_intervalAllocatedRBs[rnti];
//     ns3::g_intervalAllocatedRBs[rnti] = 0; 

//     unifiedFile << Simulator::Now().GetSeconds() << ","
//                 << rnti << ","
//                 << pastTrafficMbps << ","
//                 << pastGfbrMbps << ","
//                 << throughputMbps << ","
...
//     if (index >= traffic.size() || index >= gbr.size()) return;
    
//     video->SetDynamicDataRate(traffic[index] / 1e6);
//     if (auto qosSched = DynamicCast<NrMacSchedulerOfdmaQos>(baseSched)) {
//         qosSched->UpdateUeDlGfbr(rnti, gbr[index]);
//     } 
//     else if (auto dppSched = DynamicCast<NrMacSchedulerOfdmaDPP>(baseSched)) {
//         dppSched->UpdateUeDlGfbr(rnti, gbr[index]);
//     }

//     Simulator::Schedule(Seconds(interval), [=]() {
//         UnifiedUpdate(video, baseSched, ueDev, sink, flowMon, classifier, tracker, traffic, gbr, index + 1, interval, unifiedName);
//     });
// }
                  ...
                  std::string filename)
{
   ...
   // 2. Change the Sort to use jsonId instead of rnti
    ...
    for (uint32_t i = 0; i < ues->size(); ++i) {
        ...
        //         uint64_t rxInInterval = stat.rxPackets - ctx.tracker.lastRxPackets;
        //         uint64_t txInInterval = stat.txPackets - ctx.tracker.lastTxPackets;
        //         packetRatePps = rxInInterval / interval;
        //         if (txInInterval > 0) dropRateInterval = static_cast<double>(txInInterval - rxInInterval) / txInInterval;
        //         if (rxInInterval > 0) intervalDelayMs = ((stat.delaySum.GetSeconds() - ctx.tracker.lastDelaySumSec) / rxInInterval) * 1000.0;
...
// void
// UnifiedUpdate(Ptr<TrafficGenerator3gppGenericVideo> video,
//               Ptr<NrMacScheduler> sched,
//               Ptr<NrUeNetDevice> ueDev,
//               Ptr<PacketSink> sink,
//               Ptr<FlowMonitor> flowMon,                 // <-- NEW
//               Ptr<Ipv4FlowClassifier> classifier,       // <-- NEW
//               UeIntervalStats *tracker,                 // <-- CHANGED
//               std::vector<uint64_t> traffic,
//               std::vector<uint64_t> gbr,
//               uint32_t index,
//               double interval,
...
//     if (!headerWritten)
//     {
//         // Added AllocatedRBs to the CSV header
//         unifiedFile << "Time(s),RNTI,TargetTraffic(Mbps),GFBR(Mbps),MeasuredThroughput(Mbps),MeasuredDelay(ms),AllocatedRBGs\n";
//         headerWritten = true;
//     }
...
//     // 2. Calculate Dynamic Delay (Using FlowMonitor Delta)
//     double intervalDelayMs = 0.0;
//     double packetRate = 0.0;
//     double dropRateInterval = 0.0;
//     if (flowMon && classifier)
//     {
//         // Get the UE's IP address to find its specific flow
//         Ptr<Ipv4> ipv4 = ueDev->GetNode()->GetObject<Ipv4>();
//         Ipv4Address ueIp = ipv4->GetAddress(1, 0).GetLocal();
...
//                 uint64_t pktsInInterval = currentRxPkts - tracker->lastRxPackets;
                
//                 // Only calculate delay if we actually received packets THIS interval
//                 if (pktsInInterval > 0) 
//                 {
//                     intervalDelayMs = ((currentDelaySum - tracker->lastDelaySumSec) / pktsInInterval) * 1000.0;
//                 }
//                 // Packet Rate (RX packets / interval)
                
//                 packetRate = pktsInInterval / interval;

//                 // Drop Rate calculation for this interval
//                 uint64_t txInInterval = stat.txPackets - tracker->lastTxPackets;
//                 if (txInInterval > 0) {
//                     dropRateInterval = (double)(txInInterval - pktsInInterval) / txInInterval;
//                 }
                
//                 // Save current state for the next interval
//                 tracker->lastRxPackets = currentRxPkts;
//                 tracker->lastDelaySumSec = currentDelaySum;
//                 tracker->lastTxPackets = stat.txPackets;
//                 break; // Found the UE's flow, exit loop
//             }
//         }
//     }

//     // 3. Fetch past targets to align logs
//     double pastTrafficMbps = (index > 0) ? (traffic[index - 1] / 1e6) : 0.0;
//     double pastGfbrMbps    = (index > 0) ? (gbr[index - 1] / 1e6) : 0.0;

//    // --- NEW: Fetch and Reset the Allocated RBGs ---
//     uint32_t rbgThisInterval = ns3::g_intervalAllocatedRBs[rnti];
//     ns3::g_intervalAllocatedRBs[rnti] = 0; // Reset to 0 for the next 1-second interval!
//     // ------------------------------------------------

//     // 4. Log everything synchronously
//     unifiedFile << Simulator::Now().GetSeconds() << ","
//                 << rnti << ","
//                 << pastTrafficMbps << ","
//                 << pastGfbrMbps << ","
...
//     // 5. Update GFBR for the next interval
//     if (index >= traffic.size() || index >= gbr.size()) return;
//     uint64_t newTraffic = traffic[index];
//     uint64_t newGbr = gbr[index];
//     video->SetDynamicDataRate(newTraffic / 1e6);
    
//     if (auto qosSched = DynamicCast<NrMacSchedulerOfdmaQos>(sched)) {
//         qosSched->UpdateUeDlGfbr(rnti, newGbr);
//     } 
//     else if (auto dppSched = DynamicCast<NrMacSchedulerOfdmaDPP>(sched)) {
//         dppSched->UpdateUeDlGfbr(rnti, newGbr);
//     }

//     // 6. Schedule next tick
//     Simulator::Schedule(Seconds(interval),
//     [=]() {
//         UnifiedUpdate(video, sched, ueDev, sink, flowMon, classifier, tracker, traffic, gbr, index + 1, interval, unifiedName);
//     });
// }
// void
...
if (!configFile.empty())
{
    ...
    // // 2. USE THIS INSTEAD: Limit the buffer to 50 KB (51200 bytes).
    // Config::SetDefault("ns3::LteRlcUm::MaxTxBufferSize", UintegerValue(500 * 1024));
    // Config::SetDefault("ns3::LteRlcAm::MaxTxBufferSize", UintegerValue(50 * 1024));

    // -------------------------------------------------------------------------
    // RLC Buffer Sizing based on 3GPP Delay Budgets
    // -------------------------------------------------------------------------

    // 1. GBR_CONV_VIDEO (RLC UM): Target 150 ms delay
    // Modeled for ~50 Mbps drain rate.
    // Formula: (50 Mbps * 0.150s) / 8 = ~937 KB
    // Config::SetDefault("ns3::LteRlcUm::MaxTxBufferSize", UintegerValue(1250000));

    // // 2. NGBR Traffic (RLC AM): Target 500 ms delay max limit
    // // Modeled for congested ~20 Mbps drain rate.
    // // Formula: (20 Mbps * 0.500s) / 8 = ~1.25 MB
    // Config::SetDefault("ns3::LteRlcAm::MaxTxBufferSize", UintegerValue(1250000));

        // 1. GBR_CONV_VIDEO (150 ms limit @ 50 Mbps)
    Config::SetDefault("ns3::LteRlcUm::MaxTxBufferSize", UintegerValue(buffersize));

    // // 2. NGBR (500 ms limit @ 50 Mbps)
    ...
    // // --- NEW: Route the launcher variables to the correct scheduler ---
    // if (auto qosSched = DynamicCast<NrMacSchedulerOfdmaQos>(baseSched)) 
    // {
    //     // Call the setter method you created inside your QoS scheduler class
    //     qosSched->SetSymbolsPerSec(qosSymbolsPerSec); 
    //     std::cout << "QoS Scheduler loaded. Symbols per sec set to: " << qosSymbolsPerSec << "\n";
    // } 
    // else if (auto dppSched = DynamicCast<NrMacSchedulerOfdmaDPP>(baseSched)) 
...
//    // --- ADD THIS SORTING LOGIC BEFORE THE SCHEDULING LOOP ---
...
//     {
//         auto &ctx = dynamicUes[i];
//         double initialRateBps = ctx.traffic[0];

//         ctx.video->SetAttribute("DataRate", DoubleValue(initialRateBps / 1e6));  // Mbps

//         // Grab the sink for THIS specific UE to track its bytes
//         Ptr<PacketSink> ueSink = sinks[i];
//         uint64_t* lastRxTracker = new uint64_t(0); 
//         UeIntervalStats *tracker = new UeIntervalStats();

//         Simulator::Schedule(Seconds(appStartTimeMs/1000.0),
//         [=]() {
//             UnifiedUpdate(ctx.video,
//                    baseSched,      
//                    ctx.ueDev,
//                    ctx.sink,           // <-- Pass the specific sink for this UE
//                    monitor,        // <-- Pass FlowMonitor
//                    classifier,     // <-- Pass Classifier
//                    tracker,        // <-- Pass the new struct
//                    ctx.traffic,
//                    ctx.gbr,
//                    0u,
//                    interval,
...
//     }
// --- THE FIX ---
// 1. Sort the vector by the actual RNTI assigned by the gNB
std::sort(dynamicUes.begin(), dynamicUes.end(), [](const UeDynamicContext& a, const UeDynamicContext& b) {
    return a.ueDev->GetRrc()->GetRnti() < b.ueDev->GetRrc()->GetRnti();
});

// // 2. Schedule the updates in the sorted order
// for (uint32_t i = 0; i < dynamicUes.size(); ++i)
// {
//     // Important: Create a local copy of the context for this specific lambda
//     UeDynamicContext ctx = dynamicUes[i]; 
//     UeIntervalStats *tracker = new UeIntervalStats();
       

//     // Get the actual RNTI assigned after attachment
//     uint16_t actualRnti = dynamicUes[i].ueDev->GetRrc()->GetRnti();
//     // If you add an 'rnti' field to your UeDynamicContext struct:
//     ctx.rnti = actualRnti; 


//     Simulator::Schedule(Seconds(appStartTimeMs/1000.0),
//     [=]() {
//         // We pass 'ctx.sink' and 'ctx.ueDev' which are now perfectly aligned 
//         // because they were bundled together before sorting.
//         UnifiedUpdate(ctx.video,
//                       baseSched,      
//                       ctx.ueDev,
//                       ctx.sink,      
...
// }
// 1. Populate RNTIs and create trackers
...
// 2. Create a pointer so the lambda doesn't copy the whole vector
// We no longer need the trackers vector here.
