...
class NrMacSchedulerOfdmaQos : public NrMacSchedulerOfdmaRR
{
  public:
    ...
    /**
     * \brief Update the target GFBR for a specific UE from the simulation script
     */
    void UpdateUeDlGfbr(uint16_t rnti, double gfbrBps);

    /**
     * \brief Set the attribute "SymbolsPerSec"
     * \param v the value to save
     */
    void SetSymbolsPerSec(double v);
    /**
     * \brief Get the attribute "SymbolsPerSec"
     * \return the value of the attribute
     */
    double GetSymbolsPerSec() const;
  ...
  private:
    ...
    double m_symbolsPerSec {14000.0}; // Default value

    // Add this map to track the incoming GFBR requests!
    std::map<uint16_t, double> m_targetGfbrDl;
};
