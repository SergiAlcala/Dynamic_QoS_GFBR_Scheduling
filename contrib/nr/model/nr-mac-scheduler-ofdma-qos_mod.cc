...
void 
NrMacSchedulerOfdmaQos::SetSymbolsPerSec(double symbols)
{
    m_symbolsPerSec = symbols;
}

double 
NrMacSchedulerOfdmaQos::GetSymbolsPerSec() const
{
    return m_symbolsPerSec;
}
                                      ...
                                      const FTResources& assignableInIteration) const
{
    ...
    // --- NEW GFBR INJECTION LOGIC ---
    // Look up if the simulation script has set a dynamic GFBR for this UE's RNTI
    auto it = m_targetGfbrDl.find(uePtr->m_rnti);
    if (it != m_targetGfbrDl.end()) 
    {
        // Apply this GFBR to all active logical channels for this UE
        for (const auto& lcg : uePtr->m_dlLCG) 
        {
            for (uint8_t lcId : lcg.second->GetActiveLCIds()) 
            {
                uePtr->UpdateUeDlGfbr(lcId, it->second);
            }
        }
    }
    // --------------------------------
...
void 
NrMacSchedulerOfdmaQos::UpdateUeDlGfbr(uint16_t rnti, double gfbrBps)
{
    // Store the GFBR requested by the simulation script
    m_targetGfbrDl[rnti] = gfbrBps;
}
